...
export const Grid = React.memo((props: GridProps) => {
    ...
    const { gridColumns, expandColumn } = React.useMemo(() => {
        ...
        sortedColumnIds.forEach((id) => {
            ...
            }else if(column.getFormattedValue(ColumnsColumns.ColCellType) === CellTypes.Button){
                const col = mapToGridColumn(column, undefined, columnName, undefined, onColumnClick);
                gridColumns.push(col);
            }
        });
...
function mapToGridColumn(
    column: ComponentFramework.PropertyHelper.DataSetApi.EntityRecord,
    datasetColumn: ComponentFramework.PropertyHelper.DataSetApi.Column | undefined,
    columnName: string,
    ...
    return {
        ...
        // Button additions
        buttonCaption: column.getFormattedValue(ColumnsColumns.ColButtonCaption),
        buttonType: column.getFormattedValue(ColumnsColumns.ColButtonStyle),
        buttonPadding: undefinedIfNullish(column.getValue(ColumnsColumns.ColButtonPadding)),
        buttonHeight: undefinedIfNullish(column.getValue(ColumnsColumns.ColButtonHeight)),
        buttonIcon: column.getFormattedValue(ColumnsColumns.ColButtonIcon),
        buttonColumn: column.getFormattedValue(ColumnsColumns.ColButtonColumn),
        buttonColumnValue: column.getFormattedValue(ColumnsColumns.ColButtonColumnValue),
        buttonComparison: column.getFormattedValue(ColumnsColumns.ColButtonComparison)
    } as IGridColumn;
