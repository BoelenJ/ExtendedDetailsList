...
import { DefaultButton, FontIcon, IColumn, IconButton, Image, IRawStyle, Link, mergeStyles, PrimaryButton } from '@fluentui/react';
...
function getCellTemplate(
    ...
    switch (columnEx.cellType?.toLowerCase()) {
        ...
        case CellTypes.Button:
            ({ isBlank, cellContents } = getButtonCell(columnEx, item, cellNavigation));
            break;
        default:
...
function getButtonCell(
    ...
    const cellText = column.buttonCaption;
    const buttonType = column.buttonType ? column.buttonType.toLowerCase() : 'default';
    const padding = column.buttonPadding ? column.buttonPadding : 0;
    const height = column.buttonHeight ? column.buttonHeight : 32;
    const iconName = column.buttonIcon ? column.buttonIcon : 'StatusCircleErrorX';

    // Get the column to compare for disabling/enabling the button.
    const buttonColumn = column.buttonColumn;
    const buttonColumnValue = column.buttonColumnValue;
    const buttonComparison = column.buttonComparison ? column.buttonComparison.toLowerCase() : 'equals';
    
    // Get the value of the column for the current item.
    const columnValue = getCellValue<string>(buttonColumn, item)[0];

    // Check if columnValue equals the buttonColumnValue.
    let actionDisabled = buttonColumnValue ? columnValue !== buttonColumnValue : false;

    if(buttonComparison === 'notequals') {
    ...
    if (buttonType === 'primary') {
        cellContents = !isBlank ? (
            <PrimaryButton style={{padding: padding, height: `${height}px`}} onClick={cellNavigation} disabled={actionDisabled}>
                {cellText}
            </PrimaryButton>
        ) : (
            <></>
